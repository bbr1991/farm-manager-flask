...
def add_contact():
    ...
    return redirect(url_for('contacts_dashboard'))
@app.route('/contacts/balances')
@login_required
@permission_required('view_contacts') # Or 'view_bookkeeping'
def contact_balances():
    """
    Calculates and displays a list of all debtors (owed to farm) and 
    creditors (owed by farm).
    """
    db = get_db()
    
    # This single, powerful query gets all contacts who have an associated 
    # account and calculates their current balance.
    all_balances = db.execute("""
        SELECT
            c.id, c.name, c.phone, c.type,
            (
    ...
    for contact in all_balances:
        # A positive balance means they owe us (Debtor)
        if contact['balance'] > 0:
            debtors.append(contact)
        # A negative balance means we owe them (Creditor)
        elif contact['balance'] < 0:
            creditors.append(contact)
            
    return render_template(
        'contact_balance.html',
        user=g.user,
        debtors=debtors,
        creditors=creditors
    )
